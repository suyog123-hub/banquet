...
from app_accounts.throttles import ChangePasswordThrottle
    ...
    def post(self, request):
        try:
            ...
            if not user.check_password(current_password):
                return validation_error_response(errors={"current_password": "Incorrect old password"})
    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                return validation_error_response(errors={"email": "User not found"})

            otp = generate_otp()
            store_otp(user.id, otp, purpose="password_reset")
            send_otp_email(user.email, otp)
            return created_response(data={"message": "OTP sent to email"})
        except Exception as e:
            logger.error(str(e), exc_info=True)
            return server_error_response()
...
class VerifyPasswordResetOTPAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if not valid:
                return validation_error_response(errors={"otp": error})
    ...
    def post(self, request):
        try:
            ...
            if not email or not new_password or not reset_token:
                return validation_error_response(errors={"error": "Email, new password, and reset token are required"})

            try:
                payload = jwt.decode(reset_token, settings.SECRET_KEY, algorithms=["HS256"])
            except jwt.ExpiredSignatureError:
                return validation_error_response(errors={"error": "Reset token expired"})
            except jwt.InvalidTokenError:
                return validation_error_response(errors={"error": "Invalid reset token"})

            if payload.get("purpose") != "password_reset":
                return validation_error_response(errors={"error": "Invalid token purpose"})
